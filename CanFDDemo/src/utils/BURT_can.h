...
#define CAN_BAUD_RATE_FD 500000
...
/// Implements CAN FD 
using Can3 = FlexCAN_T4FD<CAN3, RX_SIZE_256, TX_SIZE_16>;

// A message in the CAN protocol format.
// 
// See https://en.wikipedia.org/wiki/CAN_bus#Data_transmission for details.
using CanMessage = CANFD_message_t;
// using CanFDMessage = CANFD_message_t;
...
class BurtCan {
	...
	public: 
		...
		void sendRaw(uint32_t id, uint8_t data[DATA_LENGTH], int length = DATA_LENGTH);
